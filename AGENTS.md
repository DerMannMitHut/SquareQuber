...
- One deliverable: `dist/index.html` (self‑contained). No external files at runtime.
...
- Dev: `npm run dev` (or `scripts/dev`). Serves `src/index.html` with HMR if configured.
...
- Build: `npm run build` (or `scripts/build`). Outputs single `dist/index.html` with all assets inlined.
...
- No external URLs in production output. Quick check: `rg -n "(https?://|<script src=|<link .*href=)" dist/index.html`.
